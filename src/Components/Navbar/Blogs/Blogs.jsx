...
const Blogs = () => {
  ...
  // ✅ ফেভারিটে নতুন item যোগ করার ফাংশন
  const handleFavorite = (blog) => {
    const exists = favorites.find((item) => item.id === blog.id);
    if (!exists) {
      setFavorites((prev) => [...prev, blog]);
  ...
  // ✅ ফেভারিট থেকে item remove করার ফাংশন
  const handleRemoveFavorite = (id) => {
    setFavorites((prev) => prev.filter((item) => item.id !== id)); // শুধু যেটা ক্লিক করা হয়নি সেগুলো রেখে নতুন list তৈরি
    toast.error("❌ Removed from Favorites!"); // Toast দেখাবে
  };
            ...
            <tbody>
              {blogs.map((blog) => (
                <Blog
                  key={blog.id}
                  blog={blog}
                  handleFavorite={handleFavorite} // Favorite ফাংশন প্রপ হিসেবে পাঠানো
                />
              ))}
            </tbody>
          </table>
